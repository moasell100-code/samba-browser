  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[460px]">
        <DialogHeader>
          <DialogTitle>{t('extensions.storeTitle')}</DialogTitle>
          <DialogDescription>{t('extensions.storeDesc')}</DialogDescription>
        </DialogHeader>
        <form onSubmit={(e) => void submit(e)} className="flex flex-col gap-3">
          <Input
            ...
            placeholder={t('extensions.storePlaceholder')}
            spellCheck={false}
          />
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={!input.trim() || busy}>
              {busy ? t('extensions.installing') : t('extensions.storeSubmit')}
            </Button>
          </div>
        </form>
      </DialogContent>
