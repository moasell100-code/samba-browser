...
export function HandoffCard(): React.JSX.Element | null {
  ...
  // 결제 키패드는 캡차와 안내가 다르다: 비밀번호를 직접 누르고 완료까지 눌러야 이어진다
  const title = handoff.kind === 'keypad' ? t('handoff.keypadTitle') : t('handoff.title')
  const body = handoff.kind === 'keypad' ? t('handoff.keypadBody') : t('handoff.body')
  const skip = handoff.kind === 'keypad' ? t('handoff.keypadSkip') : t('handoff.skip')
  return (
    <div
      role="alertdialog"
      aria-label={title}
