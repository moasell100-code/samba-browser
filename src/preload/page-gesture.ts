...
export function installGestureRecognizer(deps: GestureRecognizerDeps): void {
  ...
  // 사라지거나(메뉴 닫힘) iframe 위로 지나갈 때도 생기므로, 좌표가 실제로 뷰포트 밖일 때만 취소한다.
  // (preload 는 document 가 생기기 전에 돌아 documentElement 에 직접 걸 수 없다)
  document.addEventListener(
    'mouseout',
    (e: MouseEvent) => {
      if (!tracking || e.relatedTarget !== null) return
      const outside =
