...
PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    # \b 대신 lookaround 사용: 한글에 바로 붙은 경우('연락처010-...')도 잡는다.
    # 뒤쪽 \d{6,} 없음 확인은 불필요(전화번호는 항상 고정 자릿수라 주문번호와 안 겹친다).
    ...
    (
        ...
        re.compile(
            ...
            # 상세주소 토큰까지만 삼킨다 — 6자리 이상 숫자(주문번호) 앞에서 멈춘다(lookahead).
            r'(?:(?!\d{6,})[^\n,]){2,40}'
        ),
    ),
    # 라벨 + 이름('수취인 홍길동', '고객명 홍길동' 꼴) — 라벨은 남기고 이름만 가린다.
    ('name', re.compile(r'(수취인|받는분|받는 분|수령인|주문자|고객명)\s*[:：]?\s*[가-힣]{2,4}')),
    # 이름 + 호칭('홍길동님' 꼴) — 이름만 가리고 호칭은 남긴다.
    ('name_suffix', re.compile(r'[가-힣]{2,4}(?=님)')),
)
