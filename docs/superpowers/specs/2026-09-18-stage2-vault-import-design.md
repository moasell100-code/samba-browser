...
포함: SQLite 로컬 저장소, 마스터 비밀번호 기반 암호화 금고, CSV/HTML 가져오기, 키마스터 화면(1Password 스타일), AI 자동 채움·자동 로그인 도구, 로그인/결제 정보 자동 저장 제안, 북마크 사이드바(기본), 자동 로그인 검증.
